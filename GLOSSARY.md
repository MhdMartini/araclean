...
|---|---|---|---|---|
| **Tashkeel** | تشكيل | Diacritics / vocalization marks | U+064B–U+0653, U+0670 | Umbrella for all vocalization marks: the range U+064B–U+0653 covers tanween, harakat, shadda, sukun & madda, plus dagger alef (U+0670). These are the pronunciation marks (ضبط / شكل) — **not** the letter-distinguishing dots (إعجام), which are part of the base letters. |
| **Harakat** (sing. **haraka**) | حركات | Short vowels | fatha U+064E, damma U+064F, kasra U+0650 | The **three** short vowels. Sukun (U+0652) — the vowelless mark — is formally the *absence* of a haraka, not a haraka; grouped with the harakat here for convenience and removed with them by default (see the `exclude_sukun` option, story 26). |
...
| **Madda** | مدّة | Madda sign (hamza + long alef) | U+0653 (combining mark); آ U+0622 (the alef-madda letter) | Two distinct things: the **combining madda U+0653** is a tashkeel mark (removed with tashkeel); the single character **آ U+0622** is the alef-with-madda — an *alef variant* folded under letter folding (آ → ا), not tashkeel removal. Marks an alef carrying hamza + a long alef (آ = hamza + alef). |
| **Dagger alef** | ألف خنجرية | Superscript alef (marks an unwritten long alef) | U+0670 | Stands in for an omitted long alef. Standard in a fixed set of common words (هٰذا، ذٰلك، لٰكن، الله، الرحمٰن) and pervasive in Qur'anic text; surfaces only when text is vocalized — the bare forms (هذا، لكن) are the normal unvocalized spelling. |
| **Qur'anic annotation marks** | علامات قرآنية | Qur'anic recitation/annotation signs | U+0656–U+065F, U+06D6–U+06ED | Small high letters, pause & prostration marks, end-of-verse signs, etc. Vocalization/recitation marks beyond core tashkeel; preserved by `CLASSICAL`, removable as a class by `SEARCH`. |
...
| **Hamza** | همزة | Glottal-stop letter/mark | ء U+0621 | Appears standalone or seated on a carrier. |
| **Alef** | ألف | The letter alef | ا U+0627 | Variants below fold to bare alef when *Linguistic folding* is opted in. |
| **Alef variants** | — | Alef with hamza above/below, madda, wasla | أ U+0623, إ U+0625, آ U+0622, ٱ U+0671 | |
